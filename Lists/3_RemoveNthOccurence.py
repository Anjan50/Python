...
try:
    ...
    while True:
        op = int(
            input(
                "1-add element in list 2-Remove Nth occurence of word , display and Exit "
            ))
        ...
        if op == 1:
            ...
            if len(OrignalList) == 0:
                ...
                N = int(
                    input(
                        "Enter the Nth occurence the word u want to remove "))
                if word in OrignalList:
                    c = [i for i, n in enumerate(OrignalList)
                         if n == word][N - 1]
                    OrignalList.pop(c)
                    print(OrignalList)
                else:
