...
for ar in arr:
    try:
        ...
        if title not in downloaded:
            os.system(
                f"ffmpeg -i \"{file_name_final}\" -ss {final_arr[1]} -to {final_arr[2]} -async 1 \"{folder_loc}\"/\"{file_name_final.replace(f'-{downloaded_vid}', '')}\""
            )
            downloaded.append(title)
        else:
            print(
                f'\"{folder_loc}\"/\"{title}-{downloaded.count(title)}.mp4\"')
            os.system(
                f"ffmpeg -i \"{file_name_final}\" -ss {final_arr[1]} -to {final_arr[2]} -async 1 \"{folder_loc}\"/\"{file_name_final.replace(f'-{downloaded_vid}', '')}-{downloaded.count(title)}.mp4\""
            )
            downloaded.append(title)
