...
def create_hamming_code(data, should_print):
    ...
    continue_loop = True
    while continue_loop:
        continue_loop, hamming = calculate_parity(parity_index, hamming,
                                                  should_print)
        parity_index += 1
    return hamming
...
def get_pretty_check(code, valids, invalids):
    ...
    for i in range(len(code)):
        if power_of_two(i + 1):
            color = colorama.Fore.LIGHTGREEN_EX if (
                i + 1) in valids else colorama.Fore.LIGHTRED_EX
...
def handle_create(code, should_print):
    ...
    if hamming is None:
        ...
        print("Hamming code of {} ({}) is {} ({})".format(
            code, len(code), get_pretty_parity(hamming), len(hamming)))
...
def handle_check(code, should_print):
    ...
    print(
        "The data word of {} ({}) is {} ({})\t(without error correction/detection)"
        .format(get_pretty_check(str_code, valid_indices, invalid_indices),
                len(code), data, len(data)))
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        help=
        'The data or code to check. Must be a binary string. Example: 11010001'
    )
    parser.add_argument(
        '-c',
        dest='check',
